...
def filter_by_relative_pos(full_df):
    ...
    for idx,row in full_df.iterrows():
        ...
        if length:
            ...
            return_v = input(f'nuccore ID is {nuccore_ID}; does not get any info, maybe wgs, please check it manually. protein id is {idx}, end at {end},start at {start}. Y/y for indicated it near the end. ')
            if return_v.lower() == 'y':
                near_end_protein.append(idx)
            else:
                pass
