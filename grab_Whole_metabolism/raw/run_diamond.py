...
cmd_template = "diamond blastp --quiet -q {infile} -o {outfile} -d {database} -p {thread}"
...
@click.command(help='run diamond aganist kegg database.')
...
def main(indir, outdir, database, num_thread, num_parallel):
    indir = abspath(indir)
    ...
    with mp.Pool(processes=num_parallel) as tp:
        for r in tqdm(tp.imap(run_cmd, cmd_list),
                      total=len(cmd_list)):
            pass
